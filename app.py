...
DB_PATH = BASE_DIR / "storage" / "dellicar.db"
...
def dashboard() -> str:
    ...
    carrozzeria_stats = db.execute("""
    SELECT
    SUM(CASE WHEN COALESCE(work_status,'Accettata')='Accettata' THEN 1 ELSE 0 END) as accettate,
    SUM(CASE WHEN work_status='In smontaggio' THEN 1 ELSE 0 END) as smontaggio,
    SUM(CASE WHEN work_status='In verniciatura' THEN 1 ELSE 0 END) as verniciatura,
    SUM(CASE WHEN work_status='In rimontaggio' THEN 1 ELSE 0 END) as rimontaggio,
    SUM(CASE WHEN work_status='In finitura' THEN 1 ELSE 0 END) as finitura,
    SUM(CASE WHEN work_status='Pronta' THEN 1 ELSE 0 END) as pronte
    FROM work_orders
    ...
    for i, w in enumerate(work_orders):
        if w["status_date"]:
            days_in_status = (date.today() - date.fromisoformat(w["status_date"])).days
        else:
            days_in_status = 0

        work_orders[i] = dict(w)
        work_orders[i]["days_in_status"] = days_in_status
    ...
    late_work_orders = [w for w in work_orders if w["days_in_status"] > 6]
    late_count = len(late_work_orders)
    ...
    return render_template(
        ...
        carrozzeria_stats=carrozzeria_stats,
...
@app.route("/work-orders/<int:id>/status", methods=["POST"])
@role_required("admin","segretaria","officina")
def update_work_status(id: int):
    new_status = request.form.get("work_status", "").strip()
    allowed = [
        "Accettata",
        "In smontaggio",
        "In verniciatura",
    ...
    if new_status not in allowed:
        return "Stato non valido", 400

    db = get_db()
    db.execute("UPDATE work_orders SET work_status=?, status_date=date('now') WHERE id=?", (new_status, id))
    db.commit()
    return redirect("/")
    
@app.route("/work-orders/<int:id>/note", methods=["POST"])
@role_required("admin", "segretaria", "officina")
def update_work_note(id: int):
