...
def dashboard() -> str:
    ...
    work_orders = db.execute(
        """
        SELECT id, plate, notes, status_date,
               COALESCE(work_status,'Accettata') as work_status
        FROM work_orders
        WHERE COALESCE(work_status,'Accettata') != 'Consegnata'
        ORDER BY id DESC
        """
    ...
    for w in work_orders:
        row = dict(w)
        if row.get("status_date"):
            row["days_in_status"] = (date.today() - date.fromisoformat(row["status_date"])).days
        else:
            row["days_in_status"] = 0
        work_orders_list.append(row)
...
def update_work_status(id: int):
    ...
    db = get_db()
    db.execute(
    "UPDATE work_orders SET work_status=?, status_date=date('now') WHERE id=?",
    (new_status, id)
)
    db.commit()
    return redirect("/")
